...
int svm_train::examine_example(int i2) {
  float alpha2, E2, r2;
  int y2;
  y2 = labels[i2];
  alpha2 = alphas[i2];
    
  if (alpha2 > 0 && alpha2 < slack_penalty)
    E2 = error_cache[i2];
  else
    E2 = (this->*classify_example)(i2) - y2;

  r2 = y2*E2;
  if ((r2 < -tolerance && alpha2 < slack_penalty) || (r2 > tolerance && alpha2 > 0)) {
    // try argmax E1-E2
    {
      int k, i1;
      float tmax;
	
      for (i1=-1, tmax=0, k=0; k<num_examples; k++) {
	if (alphas[k] > 0 && alphas[k] < slack_penalty) {
	  float E1 = error_cache[k];
	...
	i1 = k % num_examples;
	if (alphas[i1] > 0 && alphas[i1] < slack_penalty) {
	  if (take_step(i1, i2))
	    return 1;
	}
      }
